...
RETIRED_NODES = {"FGTAM", "FGTAG", "CNTAD", "TRVAI", "CRZAA", "STRAC", "STRABD", "STRABN", "AREBC"}
...
def fetch_remote_bytes(url: str, token: str=""):
    headers={"User-Agent":"Painel-Nodes-Pelotas/1.0"}
...
def fetch_bairros_geojson():
    ...
    try:
        ...
        r=requests.get(BAIRROS_API, params=params, timeout=30, headers={"User-Agent":"Painel-Nodes-Pelotas/1.0"}); r.raise_for_status()
        js=r.json()
...
def github_headers(token): return {"Authorization":f"Bearer {token}","Accept":"application/vnd.github+json","X-GitHub-Api-Version":"2022-11-28","User-Agent":"Painel-Nodes-Pelotas/1.0"}
